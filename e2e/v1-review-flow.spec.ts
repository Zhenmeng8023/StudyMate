...
test("review page can jump back to updated graph feedback after rating a graph-sourced card", async ({ page }) => {
  await page.addInitScript((storedSession) => {
    window.localStorage.setItem("studymate.session", JSON.stringify(storedSession));
  }, session);
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (url.pathname === "/api/v1/cards/card-graph-1/review") {
      await route.fulfill({
        contentType: "application/json",
        body: success({
          reviewId: "review-graph-1",
          schedule: {
            cardId: "card-graph-1",
            userId: "user-1",
            dueAt: "2026-06-06T12:00:00Z",
            intervalDays: 4,
            easeFactor: 2.5,
            repetitionCount: 2,
            lapseCount: 0,
    ...
    if (url.pathname === "/api/v1/graphs/graph-1") {
      await route.fulfill({
        ...
        body: success({
          id: "graph-1",
          ownerUserId: "user-1",
          title: "Graph feedback demo",
          description: "review back to graph feedback",
          visibility: "private",
          status: "active",
          graphType: "knowledge",
          mode: "free",
          ...
          edgeCount: 0,
          createdAt: "2026-06-02T12:00:00Z",
          updatedAt: "2026-06-02T12:12:00Z",
          document: {
            graphId: "graph-1",
            version: 4,
            schemaVersion: 1,
            ...
            nodes: [
              {
                ...
                width: 220,
                height: 120,
                source: { type: "graph", id: "node-1", label: "Binary Tree" },
                metadata: {
                  reviewFeedback: {
                    sourceType: "graph",
                    sourceId: "node-1",
                    ...
                    learningCount: 0,
                    maxLapseCount: 0,
                    sampleCardFronts: ["Binary tree traversal"]
                  }
                }
              }
            ],
  ...
  const backlink = page.getByRole("link", { name: "回到图谱查看反馈" });
  await expect(backlink).toBeVisible();
  await backlink.click();

  await expect(page.getByText("Graph feedback demo")).toBeVisible();
  await page.getByLabel("打开检查器").click();
  await page.locator(".graph-node", { hasText: "Binary Tree" }).click();
  await expect(page.getByText("掌握度 100% · 已掌握")).toBeVisible();
});
