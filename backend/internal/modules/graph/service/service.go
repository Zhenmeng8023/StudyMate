	...
	return []graphdto.DiagramTemplatePayload{
		{
			...
			Name:        "UML 时序图",
			Category:    "uml",
			Description: "适合表达调用顺序、服务间交互和请求链路。",
			Mode:        "diagram",
			SampleLines: []string{"Client -> API", "API -> Service", "Service -> DB"},
		},
		{
			ID:          "erd-core",
			Name:        "ERD 数据关系",
			Category:    "erd",
			Description: "适合梳理核心实体、主外键和聚合边界。",
			Mode:        "diagram",
			SampleLines: []string{"users", "materials", "notes", "graphs"},
		},
		{
			ID:          "c4-context",
			Name:        "C4 Context",
			Category:    "c4",
			Description: "适合从系统上下文开始画软件架构草图。",
			Mode:        "diagram",
			SampleLines: []string{"User", "StudyMate", "MySQL", "MongoDB"},
		},
	}
}
