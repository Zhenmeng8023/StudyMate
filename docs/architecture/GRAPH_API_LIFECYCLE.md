...
- 冲突辅助卡片现在还支持 `应用已标记取舍到当前草稿`：会以最新图谱 head 作为版本基线，保留当前本地标题/说明/视口，并把已标记为 `保留本地` 的节点、连线、分组对象覆盖回 rebased draft，使其能继续以最新版本号直接保存
- 在显式应用这些对象级取舍后，工作区状态栏还会立刻返回可解释的结果摘要，例如本次保留了多少本地对象、沿用了多少服务端对象，以及多少对象虽标记为“稍后处理”但当前草稿已经沿用最新版本
- 在真正应用这些对象级取舍前，前端还会额外执行最小结构依赖校验：如果合并草稿会留下 `dangling_edge` 或 `invalid_group_node` 这类跨对象依赖错误，卡片会直接列出阻断问题，并要求用户先补齐相关节点或改为保留服务端
- 当用户只标记了部分对象级取舍时，冲突卡片还会额外统计“未标记对象”数量，并明确提示：如果现在直接应用，未标记对象会默认沿用最新图谱版本；相同说明也会同步进入冲突摘要/处理包的人工合并清单
- 当依赖校验确实触发阻断时，冲突卡片现在还会直接生成联动取舍建议：例如为 dangling edge / invalid group node 推荐“联动保留本地依赖节点”或“改为保留服务端”，让用户不必自己回到对象列表逐项查找
- 对于多目标连线，`dangling_edge` 联动建议现在也会把 `metadata.targetNodeIds` 中的附加依赖节点一并纳入，而不再只关注主 `sourceNodeId / targetNodeId`
- 当同一轮阻断对应多条联动取舍建议时，冲突卡片现在还允许直接执行 `一键应用 N 项联动取舍建议`：批量把这些建议写成当前对象级取舍标记，再交由用户统一确认是否继续应用到最新 head
- 批量标记这些联动建议后，工作区状态栏现在还会立即返回带预检结论的反馈：不仅说明本次标记了多少“保留本地 / 保留服务端 / 稍后处理”，还会同步告知“当前已解除依赖阻断，可继续应用”或“仍有 N 个依赖问题待处理”
- 当阻断尚未完全解除时，这条批量反馈现在还会补一段精简的剩余阻断对象摘要，例如“连线“Local edge”会引用未保留的节点”，帮助用户快速判断下一步还需要继续调整哪些对象
- 在真正点击“应用已标记取舍到当前草稿”前，冲突卡片里的“取舍依赖校验问题”区块现在也会先显示同一套阻断摘要，而不是只展示逐条明细列表；最终预检兜底文案和卡片摘要因此保持一致
- 同一组“取舍依赖校验问题”的逐条明细标题现在也会优先显示可读对象名，例如“连线“Local edge””或“节点“Broken source node””，而不是裸 `targetId`
- 冲突卡片现在还会额外展示一条“应用前预检”摘要：直接告诉用户“如果现在应用”会保留哪些取舍，或者会被哪些依赖问题阻断，把最终决策前的结果预览前移到卡片内
- 这条“应用前预检”摘要现在还会把“未标记对象会默认沿用最新图谱版本”的结果一并说清楚，让用户在最终点击前就能同时看到“已标记会怎么合并”和“未标记会怎么回退”
- 这条“应用前预检”摘要现在还会额外带上代表对象示例，例如“保留本地：本地节点 / 保留服务端：旧关系”，避免用户只看到数量却仍不知道这次取舍主要指向哪些对象
- 这些阻断摘要现在会优先使用校验器生成的可读短原因，例如“连线“Local edge”会引用未保留的节点”，只在 message 仍是泛化短码时回退到 `targetId` 或 `ruleType`
- 对于 `invalid_source_target` / `invalid_node_size` 这类节点级错误，冲突卡片现在也会给出直接的 `keep-latest` 建议：当本地节点来源不完整或尺寸非法时，用户可以先回到服务端版本，再决定是否重新整理本地草稿；对应阻断 message 与建议文案也统一为中文
- 这一层显式应用动作仍然不替代真正的自动合并；它的职责是把人工合并前的对象级判断落成一份可保存草稿，而不是在后台静默解决所有冲突
...
4. 后续若要进入完整冲突解决流，仍需要在这层“未标记对象提示 + 最小跨对象依赖校验 + 联动取舍建议/批量标记”之上继续补更完整的联动取舍策略、合并策略或更强的自动辅助能力
