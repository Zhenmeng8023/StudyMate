...
- 大型工作区 hook 已继续拆出 autosave/beforeunload、右键菜单关闭、stage 测量生命周期 hook、header/source rail/toolbar 纯视图组件，以及 stage status/world/minimap 空态视图组件；后续仍需把数据加载、画布交互状态机和 detail rail 完整拆到独立容器与纯视图。
