...
- `frontend-admin/src/views/AdminWorkspaceView.vue` 已开始从单体工作台向模块壳层收口：最基础的请求边界与后台 session store 已抽到 `frontend-admin/src/api/client.ts` 和 `frontend-admin/src/api/sessionStore.ts`，启动时 refresh 失败会显式清空本地会话并回退登录页；`frontend-admin/src/router/index.ts` 已提供规范化的 `/admin/dashboard`、`/admin/moderation`、`/admin/users` 等路径映射；当前又进一步拆出了 `frontend-admin/src/views/modules/AdminDashboardModule.vue`、`AdminModerationModule.vue` 与 `AdminGovernanceModule.vue` 三个首批模块视图，让概览、审核与治理列表先脱离单文件内联模板。后台目前仍未进入真正的 Vue Router page 级拆分，但 `AdminWorkspaceView.vue` 已开始回到“session / URL / 确认层壳组件”的角色。
