...
- `frontend-admin/src/views/AdminWorkspaceView.vue` 约 22KB，后台仍主要是单工作台组件内切换模块；不过最基础的请求边界与后台 session store 已开始抽到 `frontend-admin/src/api/client.ts` 和 `frontend-admin/src/api/sessionStore.ts`，启动时 refresh 失败也会显式清空本地会话并回退登录页。`frontend-admin/src/router/index.ts` 现已提供规范化的 `/admin/dashboard`、`/admin/moderation`、`/admin/users` 等路径映射，`AdminWorkspaceView.vue` 也会同步浏览器 URL、支持直接进入子模块和返回/前进，但页面仍未拆成真正独立的模块页与完整 Vue Router 组合。
