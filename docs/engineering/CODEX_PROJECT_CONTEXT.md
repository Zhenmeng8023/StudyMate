...
5. **设计系统与 API 契约仍未完全成为共享底座**
   设计 token、基础 UI 组件与页面状态协议虽已起步，前后台 API 的 request/error/auth-header、query/pagination 与 JSON 请求体编码也已开始沉到共享层，但 pagination 响应契约、auth-session、401 生命周期与更多后台模块请求边界仍分散在各端实现；任何新页面和后台模块继续扩展前，应先收口这些共享边界。
