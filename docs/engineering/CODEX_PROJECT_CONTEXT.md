...
共享包：`@studymate/graph-core` 已存在并有独立测试命令；`@studymate/ui` 已开始承接共享页面状态契约，并进一步收口了 `DataState`、`Drawer`、`Inspector`、`IconButton`、`Button`、`Tag`、`Input`、`Select` 八个基础 primitive，但整体仍处于起步阶段；`@studymate/api-client` 已开始承接共享 request/error/auth-header、query/pagination 与 JSON 请求体编码基础层，`@studymate/editor-core` 仍接近占位状态，跨前后台的稳定契约层仍需继续收口。
