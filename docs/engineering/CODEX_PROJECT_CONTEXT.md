...
- Deck/Card、SM-2 调度、今日队列、复习提交与图谱/笔记/AI 的局部闭环已存在。
- 复习模块的目标应升级为类似 Anki 的闪卡系统，而不是简单“到期卡片翻面复习”：需要逐步补齐 Note / Card 分离、模板生成卡片、Cloze 填空、牌组配置、学习/复习/重新学习队列、暂停/埋藏、标签、浏览器和来源回跳。
...
| Phase 6 | 局部预埋 | 复习反馈、AI 草稿、图谱出卡已有接口与局部实现；下一步复习侧应从简单 SM-2 队列升级为 Anki 式闪卡系统，再承接图谱/笔记反馈闭环。 |
| Phase 7 | 暂不进入主线 | PlantUML/SQL/OpenAPI 深度导入、代码分析、Tauri 不应抢占 P0/P1。 |

### 2026-07-15 快速原型执行校准

对照根目录《学伴项目-设计说明书.md》后，当前执行策略调整为：先把“资料/PDF -> 批注 -> 笔记/图谱 -> 卡片 -> 复习 -> Dashboard/图谱薄弱反馈”做成可演示闭环，再补齐 Anki 正式模型、图谱熟练度回写、搜索产品化和后台治理细节。

这意味着：

- `LC-010 + ANKI-060` 优先级上移，先做最小学习反馈闭环。
- `ANKI-000/010/020` 仍必须补，但定位为兼容现有 Card 原型的正式模型归并，不阻塞 `ANKI-030/040/050/070` 的可用化切片。
- `GPH-040` 继续拆图谱控制器，但只优先拆会阻碍下一步主闭环交付的 store / commands / features。
- `SE-020` 和 `ADM-011` 优先服务主路径可信度：搜索结果可继续、可跳转、可解释；后台治理有备注、审计和失败提示。
- 课程/LMS、Tauri、完整工程图谱、向量/RAG、完整 `.apkg` 兼容继续后置。
- 任何后端业务能力更新都必须同步评估前端验收面：页面入口、状态展示、错误提示、空态和回归路径要跟上后端实现；除纯内部重构/安全修复外，不接受后端 API 长期领先而前端无可视验收入口。
...
7. **复习卡片目标需要从“简单复习”校准为 Anki 式闪卡系统**
   当前 `card` 模块已有 `Deck / Card / Schedule / Review` 与 `again / hard / good / easy` 的 SM-2 调度，但仍缺少 Anki 的核心产品模型：Note 与 Card 分离、模板批量出卡、Cloze、学习步进、重新学习、暂停/埋藏、标签、浏览器和导入导出边界。后续 `WB-033 / LC-010` 不应只做“今日复习 UI 加厚”，而应围绕 Anki mental model 建立 StudyMate 自己的记忆系统。

### P1

1. 图谱内核继续下沉到 `packages/graph-core`，并把浏览器状态进入 store、用户意图进入 commands，减少工作区控制器逻辑。
2. 图谱-复习反馈仍需从“局部连接”升级为“稳定回写闭环”，并服务于“资料上传 -> 阅读批注 -> 笔记块 -> CardNote -> 模板生成闪卡 -> Anki 式复习 -> 图谱熟练度回写”的主演示路径。
...
- 后端相关代码发生用户可见或管理员可见能力变更时，前端必须同步暴露最小可验收入口；若临时无法同步，必须在 `PROJECT_LOG.md` 写明原因、风险、补齐任务和预计顺序。
- 任何破坏性接口或迁移都必须带兼容说明。
