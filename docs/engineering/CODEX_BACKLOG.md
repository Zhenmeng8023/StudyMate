...
## 2026-07-15 快速原型优先级校准

本轮按根目录《学伴项目-设计说明书.md》复核后，执行重心调整为“先跑通主演示闭环，再补齐专业深度”。说明书中的全项目标杆继续保留，但当前不再平均推进课程、桌面端、完整工程图、向量/RAG、完整 Anki 兼容等重能力。

当前最近 2-4 个工作包的建议顺序：

| 顺位 | 工作包 | 目标 | 快速原型验收 |
|---|---|---|---|
| 1 | LC-010 + ANKI-060 | 主学习闭环与薄弱反馈 | 用户能从资料/PDF 批注或图谱节点生成卡片，复习后在 Dashboard 或图谱侧看到薄弱反馈入口。 |
| 2 | ANKI-000 -> ANKI-010 -> ANKI-020 | Anki 式模型兼容层 | 不推翻现有 Card；先补 CardNote / NoteType / Template / 队列状态的兼容设计和最小 Basic/Reverse/Cloze 落地。 |
| 3 | WB-033 | 图谱-复习反馈回写 | 先回写图谱节点或来源级 mastery/weakness 摘要；复杂学习画像后置。 |
| 4 | SE-020 + ADM-011 | 搜索与后台服务主路径 | 搜索补空结果建议、来源跳转与稳定续取；后台补治理备注、审计可读性和失败提示。 |
| 5 | GPH-040 | 防止图谱控制器继续膨胀 | 只拆会阻碍下一步闭环功能的 store/commands/features，不为了纯架构完整暂停产品切片。 |

纠偏规则：

- 已经进入 `IN_PROGRESS` 的复习原型能力不再被 `ANKI-010/020` 严格阻塞；后续由正式模型兼容归并。
- 新增功能必须回到“资料/阅读 -> 批注 -> 笔记/图谱 -> 卡片 -> 复习反馈”之一，否则默认后置。
- 修改或新增后端业务能力时，前端必须同步给出最小可视验收入口、状态展示和错误/空态处理；除纯内部重构/安全修复外，不允许长期只交付 API 而没有页面可验收。
- Meilisearch、工程图谱、Tauri、课程/LMS、向量数据库和 `.apkg` 深兼容只保留设计入口，不进入当前冲刺。
...
| ANKI-000 | TODO | Anki 式闪卡产品契约 | FE-030, LC-010 | docs/engineering、docs/architecture、card/review domain | 明确 Deck、CardNote、NoteType、CardTemplate、GeneratedCard、Schedule、ReviewLog、Tag、SourceLink 的职责边界；输出兼容现有 `Card(front/back)` 的迁移路线，确认哪些 Anki 能力进入快速原型，哪些仅预研。 |
| ANKI-010 | TODO | Note / Card 分离与模板生成 | ANKI-000 | backend card models/dto/service、frontend review types | 不重写当前卡片链路，先支持一条 CardNote 通过模板生成一张或多张 Card；首批模板覆盖 Basic、Basic Reverse、Cloze；旧 `front/back` 卡片必须可兼容读取。 |
| ANKI-020 | TODO | Anki 式调度与队列模型 | ANKI-000 | card schedule service/repository/review API | 在现有 SM-2 与 `active/suspended/buried` 原型上补 new / learning / review / relearning 状态、学习步进、每日新卡/复习上限和重新学习路径；继续保留 `again / hard / good / easy` 评分语义。 |
...
| ANKI-060 | IN_PROGRESS | 复习反馈回写学习图谱 | ANKI-050, LC-010 | graph/card/review/dashboard | 当前已起步补上 `GET /review/feedback` 摘要接口，并让 dashboard 直接展示薄弱卡片、学习中卡片与到期卡片数量，作为学习反馈入口；本轮已继续补上来源级 `weakSources` 汇总，并让图谱选中节点在来源匹配时直接展示“待回补卡片 / 到期 / 学习中 / 遗忘次数”反馈，且能带着 `sourceType + sourceId` 精确跳回复习工作台；进一步又补上 `sourceSummaries` mastery 摘要，让图谱节点即使没有弱项也能看到“掌握度 / 已进入稳定复习卡片数 / 最高遗忘次数”等来源级学习反馈；现在复习页在 graph 来源卡片评分后也会直接出现“回到图谱查看反馈”入口，便于在同一会话里验证图谱侧反馈，而图谱侧也已补上“刷新学习反馈”入口，能够在不覆盖本地未保存编辑的前提下拉取最新 mastery 摘要并覆盖过期快照；后续再继续补图谱节点或来源级 mastery 的更持久回写，复杂学习画像和完整 Anki 队列统计后置。 |
...
| SE-020 | IN_PROGRESS | MySQL fallback 搜索服务端分页与真实统计 | WB-014 | search service/handler/frontend search | `GET /search` 支持 cursor/limit/sort 或等价分页；每类结果有真实命中数、搜索耗时、排序语义、空结果建议和来源跳转契约。当前响应已显式透出 `limit`、`elapsedMs` 与按组 `nextOffset`，用户端搜索页在“全部类型”与单一类型视图下都可直接继续请求某个分组的下一批结果；后续继续补更完整的跨批次服务端分页。 |
...
### 执行记录：WB-033 / LC-010（图谱侧刷新学习反馈并保留本地草稿）
- 执行日期：2026-07-16
- 关联提交：待提交
- 实际变更：
  - 更新 `frontend-user/src/modules/graph/hooks/useGraphWorkspaceController.tsx`
  - 更新 `frontend-user/src/modules/graph/components/GraphWorkspaceSelectionPanel.tsx`
  - 更新 `frontend-user/src/modules/graph/lib/graphSourceReviewFeedback.ts`
  - 更新 `frontend-user/src/modules/graph/lib/graphSourceReviewFeedback.test.ts`
  - 更新 `frontend-user/src/modules/graph/GraphWorkspacePage.test.tsx`
- 完成证据：
  - 图谱节点检查器里的学习反馈卡片现在新增“刷新学习反馈”入口，用户在复习页完成评分后回到图谱工作区，可以不重载整张图谱就拉取最新来源级 mastery 摘要。
  - 新拉取的 `sourceSummaries / weakSources` 现在会在命中同一来源节点时优先覆盖旧的 `metadata.reviewFeedback` 快照，避免图谱详情初次读入时附带的过期反馈继续遮蔽最新复习结果。
  - 刷新动作只更新图谱侧的 review feedback 读模型，不改写 graph document 节点内容，也不重置本地未保存标题等脏编辑，保证自动保存、脏状态提示和后续持久化边界不被污染。
- 已执行验证：
  ...
  - `npm run build:user`
- 后续影响：
  - `WB-033 / LC-010` 现在不只具备“复习完成后跳回图谱”能力，也具备“回到图谱后主动拉取最新反馈而不丢草稿”的最小即时刷新语义，主演示闭环更接近真实可用。
  - 这一轮仍然没有把 mastery 正式持久化到图谱领域字段，也没有做跨端推送或后台自动失效；如果继续推进，更适合优先定义持久化边界、刷新失效策略与冲突提示，而不是把运行时反馈继续散落在更多页面局部状态里。
...
- 实际变更：
  ...
  - 更新 `frontend-user/src/modules/graph/lib/graphSourceReviewFeedback.ts`
  - 新增 `frontend-user/src/modules/graph/lib/graphSourceReviewFeedback.test.ts`
- 完成证据：
  - `GET /graphs/:id` 现在会在返回详情时，为带来源的图谱节点内嵌 `metadata.reviewFeedback`，把最新来源级 mastery 摘要直接带回图谱工作区。
  - 图谱保存、导入与快照恢复前会先剥离运行时 `reviewFeedback` 字段，避免把这类读时回填数据误写进 graph document、版本快照或后续冲突合并草稿。
  - 前端图谱侧现在会优先消费节点自带的 `reviewFeedback`，只有缺失时才回退到全局 `/review/feedback` 汇总，节点选中后的反馈落点更接近真实读链路。
- 已执行验证：
...
- 后续影响：
  - `WB-033 / LC-010` 现在从“图谱页面额外拉一份反馈摘要”推进到“图谱详情自身就携带节点级 mastery 快照”，后续更适合继续补复习后即时刷新或更明确的持久化出口。
  - 这一轮仍然没有把 mastery 作为正式领域字段写入图谱文档；后续如果继续推进，应该优先定义持久化边界与兼容策略，而不是让运行时元数据自然扩散。

### 执行记录：WB-033 / LC-010（图谱节点来源级 mastery 摘要起步）
- 执行日期：2026-07-15
- 关联提交：`9dbe769`（RED）
- 实际变更：
  - 更新 `backend/internal/modules/card/dto/card.go`
  - 更新 `backend/internal/modules/card/repository/repository.go`
  - 更新 `backend/internal/modules/card/service/review_feedback.go`
  ...
  - 更新 `frontend-user/src/modules/graph/hooks/useGraphWorkspaceController.tsx`
  - 更新 `frontend-user/src/modules/graph/components/GraphWorkspaceSelectionPanel.tsx`
  - 更新 `frontend-user/src/modules/graph/components/GraphWorkspaceSelectionPanel.test.tsx`
  - 更新 `e2e/v1-graph-workspace.spec.ts`
- 完成证据：
  - `GET /review/feedback` 现在除 `weakSources` 外，还会返回覆盖所有已关联来源的 `sourceSummaries`，包含 `totalCardCount`、`reviewCardCount`、`masteredCardCount`、`masteryLevel` 与 `masteryScore`。
  - 图谱节点检查器现在不只会显示“待回补卡片”，即使来源下没有弱项，也能显示“掌握度 / 已进入稳定复习卡片数 / 最高遗忘次数”等来源级学习反馈。
  - 浏览器级 smoke 已覆盖图谱工作区选中来源节点后展示 mastery 反馈卡片，说明这条学习反馈回写已经抵达真实页面而不只停在组件测试。
- 已执行验证：
...
- 后续影响：
  - `WB-033` 已经从“只有弱项入口”推进到“开始回写来源级掌握度摘要”，图谱节点对学习状态的承接更接近真正闭环。
  - 下一步更适合继续把这份来源级 mastery 往节点持久字段、dashboard 同步摘要或复习后即时刷新路径推进，而不是回到新增孤立入口。

### 执行记录：LC-010 / ANKI-060（图谱节点来源级精准回补）
- 执行日期：2026-07-15
- 关联提交：`39e83a3`（RED）
- 实际变更：
  - 更新 `backend/internal/modules/card/dto/card.go`
  ...
  - 更新 `frontend-user/src/modules/review/ReviewWorkspacePage.test.tsx`
- 完成证据：
  - 图谱节点检查器里的“打开复习工作台”入口现在会带上来源级 `sourceType + sourceId`，而不是只做笼统跳转。
  - 复习工作台能够消费 `/review?sourceType=...&sourceId=...`，跨牌组定位首个命中的来源卡片集合，并自动打开卡片管理视图以便继续回补。
  - 卡片浏览器后端列表接口与前端筛选状态都已补上 `sourceId` 过滤，来源级回补和手动排查共用同一套最小查询语义。
- 已执行验证：
  ...
  - `npm run verify:docs`
- 后续影响：
  - `ANKI-060 / LC-010` 现在不只是在图谱里展示反馈，还能把用户稳定带到“这一来源下该复习什么”的最小工作面。
  - 下一步更适合继续补 `WB-033` 的 mastery 持久回写，或把来源级过滤进一步沉淀成更统一的 SourceLink / review filter 契约，而不是回到新增孤立入口。

### 执行记录：ANKI-060 / LC-010（图谱节点来源级复习反馈）
- 执行日期：2026-07-15
- 关联提交：`b3f32e4`（RED）
- 实际变更：
  - 更新 `backend/internal/modules/card/dto/card.go`
  - 新增 `backend/internal/modules/card/service/review_feedback.go`
  - 更新 `backend/internal/modules/card/service/service.go`
  - 更新 `backend/internal/modules/card/service/list_cards_filters_test.go`
  - 更新 `backend/internal/modules/card/handler/handler_test.go`
  - 更新 `frontend-user/src/api/types.ts`
  - 新增 `frontend-user/src/modules/graph/lib/graphSourceReviewFeedback.ts`
  - 更新 `frontend-user/src/modules/graph/hooks/useGraphWorkspaceController.tsx`
  - 更新 `frontend-user/src/modules/graph/components/GraphWorkspaceSelectionPanel.tsx`
  - 更新 `frontend-user/src/modules/graph/components/GraphWorkspaceSelectionPanel.test.tsx`
- 完成证据：
  - `GET /review/feedback` 现在除卡片级 `weakCards` 外，还会返回按 `sourceType + sourceId` 聚合的 `weakSources`，至少覆盖待回补卡片数、到期数、学习中数、最高遗忘次数与示例卡片正面。
  - 图谱工作区会以容错方式单独拉取这份反馈，并在选中节点来源与 `weakSources` 命中时，于检查器里直接展示来源级复习反馈，不阻断图谱主加载。
  - 节点检查器里的反馈会保留“打开复习工作台”入口，让用户从图谱节点继续回到复习工作区处理薄弱卡片。
- 已执行验证：
  - RED：`go test ./internal/modules/card/handler ./internal/modules/card/service`
  - RED：`npm --workspace frontend-user run test -- src/modules/graph/components/GraphWorkspaceSelectionPanel.test.tsx`
  - GREEN：`go test ./internal/modules/card/handler ./internal/modules/card/service`
  - GREEN：`npm --workspace frontend-user run test -- src/modules/graph/components/GraphWorkspaceSelectionPanel.test.tsx`
- 后续影响：
  - `ANKI-060 / LC-010` 现在已经从 dashboard 摘要继续推进到图谱节点检查器，学习反馈开始真正回到图谱语义入口。
  - 下一步更适合继续补 `WB-033` 的节点熟练度 / 来源级 mastery 持久回写，或让复习工作台支持更精确的来源过滤回补，而不是回到只做首页摘要。
...
- 执行分支/提交：`master` / 待提交
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspacePageFeature.ts`
  - 新增 `frontend-admin/src/views/adminWorkspacePageFeature.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
- 完成证据：
  - 新的 `adminWorkspacePageFeature` 统一聚合 `state / feature / confirm / interaction / surface` 五层编排，`AdminWorkspaceView.vue` 不再直接组装 request 边界、adapter 依赖与 reset controller。
  - `AdminWorkspaceView.vue` 现在只保留 `workspacePageFeature` 创建、runtime 挂载/卸载和 page surface 透传，文件进一步降到 34 行，已经接近纯 route container。
  - page feature 单测锁定了状态适配器、feature adapter、confirm adapter、interaction adapter、surface adapter 的接线顺序，并验证 reset controller 会继续绑定到 `confirm.resetAll`。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从 page surface 推进到 page feature，后台工作台页面本体已经基本只剩 route 容器职责。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估真正的 Router 页面拆分或 route-level feature 边界，而不是重新把 adapter 编排写回 `AdminWorkspaceView.vue`。

### 执行记录：FE-041（管理端 workspace page surface 接线）
- 执行日期：2026-07-15
- 执行分支/提交：`master` / 待提交
- 实际变更：
  - 新增 `frontend-admin/src/views/AdminWorkspacePageSurface.vue`
  - 新增 `frontend-admin/src/views/AdminWorkspacePageSurface.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
- 完成证据：
  - 新的 `AdminWorkspacePageSurface` 直接消费 `workspaceSurface`，统一承接 `AdminConfirmStack`、`AdminLoginPanel`、`AdminShellFrame` 与 `AdminWorkspaceModuleHost`，页面模板不再继续拆成多组 surface computed 与局部 confirm handler。
  - `AdminWorkspaceView.vue` 现在只保留工作台状态、feature adapter、runtime 与 `workspaceSurface` 组装，文件进一步降到 308 行，更接近真正的 route-level page container。
  - page surface 单测锁定了两条主分支：未登录分支会透传确认弹层与登录事件；已登录分支会透传壳层导航/刷新/退出，以及 governance 模块查询与治理动作。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从 module host 推进到 page surface，后台工作台页面本体已经基本只剩状态与 feature 组合职责。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估 route-level page feature 或真正的 Router 页面拆分，而不是把登录/壳层模板重新写回 `AdminWorkspaceView.vue`。

### 执行记录：FE-041（管理端 workspace module host 接线）
- 执行日期：2026-07-15
- 执行分支/提交：`master` / 待提交
- 实际变更：
  - 新增 `frontend-admin/src/views/modules/AdminWorkspaceModuleHost.vue`
  - 新增 `frontend-admin/src/views/modules/AdminWorkspaceModuleHost.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
- 完成证据：
  - 新的 `AdminWorkspaceModuleHost` 统一承接 dashboard / moderation / governance 三段模块切换模板，页面层不再直接内联三路 `v-if / v-else-if / v-else` 分支。
  - `AdminWorkspaceView.vue` 现在只把 `activeView`、`moduleProps` 和 `moduleEvents` 交给 host，继续沿 `FE-041 / ADM-010` 回到 page surface 角色。
  - 模块 host 单测锁定了三条渲染路径和事件透传：dashboard 的 `openModeration`、moderation 的筛选与审核动作、governance 的筛选、选中与治理动作都会按既有 surface 正常转发。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从 workspace surface adapter 推进到更薄的一层 module host，后台工作台模板层围绕模块切换的最后一段分支也开始从页面本体抽离。
...
- 执行分支/提交：`master` / 待提交
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceChromeAdapter.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceChromeAdapter.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
  - 更新 `PROJECT_LOG.md`
- 完成证据：
  - 后台工作台现已新增共享 `adminWorkspaceChromeAdapter`，把登录面板与后台壳层的 `props / events` 以及它们依赖的导航分组、会话提示、计数文案等外壳派生状态收口到统一出口，而不是继续由 `AdminWorkspaceView.vue` 分别拼装 login panel props/events、shell props/events 与辅助导航状态。
  - `AdminWorkspaceView.vue` 现在通过 `chromeBindings` 消费 `loginPanelProps / loginPanelEvents / shellProps / shellEvents`，页面壳层里围绕外壳绑定的样板代码进一步收口，文件当前降到 576 行。
  - 新增 chrome adapter 单测，锁定“会话失效提示与 notice fallback 会按当前共享契约组合”“shell 导航分组、计数文案与动作事件会按 active view 透传”两组外壳绑定契约。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从 confirm / interaction adapter 推进到更高一层的 chrome adapter，后台工作台页面壳层里围绕登录与壳层骨架的绑定编排进一步变薄。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估模块 props/events、页面派生状态与 reset 协调是否继续进入更稳定的工作台 feature 边界，而不是重新把这些外壳绑定逻辑写回 `AdminWorkspaceView.vue`。
...
- 执行分支/提交：`master` / 待提交
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceFeatureAdapter.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceFeatureAdapter.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `frontend-admin/src/views/adminWorkspaceActionAdapter.ts`
  - 更新 `frontend-admin/src/views/adminWorkspaceReadAdapter.ts`
  - 更新 `frontend-admin/src/views/adminWorkspaceMutationAdapter.ts`
  - 更新 `frontend-admin/src/views/adminWorkspaceRuntime.ts`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
  - 更新 `PROJECT_LOG.md`
- 完成证据：
  - 后台工作台现已新增共享 `adminWorkspaceFeatureAdapter`，把 `read / mutation / action / runtime` 四组适配层接线进一步汇总到同一 feature 出口，而不是继续由 `AdminWorkspaceView.vue` 分别创建四套 adapter。
  - `AdminWorkspaceView.vue` 现在通过 `workspaceFeature.read / mutations / actions / startRuntime()` 消费工作台能力，页面层围绕 adapter 装配的样板代码进一步收口，文件当前降到 627 行。
  - 新增 feature adapter 单测，锁定“action 和 mutation 会复用同一 read adapter 暴露的 loader / refreshProfile”“runtime 会复用同一 read adapter 暴露的 loadActiveView / refreshProfile”两条组合契约。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从共享 runtime / action / read / mutation 推进到更高一层的 workspace feature adapter，后台工作台页面壳层进一步回到组合与展示角色。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估工作台剩余的派生状态与模块装配是否继续纳入这一 feature 出口，而不是重新把 adapter 组合逻辑写回 `AdminWorkspaceView.vue`。
...
- 实际变更：
  ...
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里原本内联维护的 `workspaceResetHandlers / clearWorkspaceState` 编排，现已收口到共享 `adminWorkspaceResetController` helper。
  - 新增 controller 单测，锁定“全量 reset 五组工作台切片”和“按指定 key 局部 reset”的组合契约。
  - `AdminWorkspaceView.vue` 当前已进一步收口到 787 行，继续维持在仓库约束内，同时既有治理页页面级回归保持通过。
- 已执行验证：
  ...
  - `npm --workspace frontend-admin run typecheck`
- 后续影响：
  - `FE-041` 现在继续从共享确认编排 controller 推进到共享 workspace reset controller，后台工作台壳层里的 reset 入口也开始复用统一出口。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估治理动作包装或更完整的工作台 feature adapter，而不是把新的 reset 编排重新写回页面层。
...
- 实际变更：
  ...
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里原本分散在页面层的五组确认弹层 `dialogs / resetHandlers / submitHandlers` 编排，现已收口到共享 `adminWorkspaceConfirmController` helper。
  - 新增 controller 单测，锁定“按最新 pending/error 生成确认弹层”和“按 confirm key 分发提交动作”的组合契约。
  - `AdminWorkspaceView.vue` 当前已从 832 行收口到 798 行，回到仓库约束内，同时既有治理页页面级回归保持通过。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从共享登录面板事件装配链推进到共享确认编排 controller，后台工作台壳层里残留的确认流组装进一步变薄。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估 `workspaceResetHandlers` 与更完整的工作台 feature adapter，而不是把新的确认流拼装重新写回页面层。

### 执行记录：FE-041（管理端登录面板事件装配 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`a00cb3a`（RED）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceLoginPanelEvents.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceLoginPanelEvents.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里传给 `AdminLoginPanel` 的 `submit / update:login-value / update:password-value` 事件转发已收口到共享 `adminWorkspaceLoginPanelEvents` helper。
  - 新增 helper 单测，锁定登录面板提交、账号输入与密码输入事件的转发契约。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认登录、自举与退出路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估更完整的登录 feature adapter，而不是重新把登录事件转发写回组件层。

### 执行记录：FE-041（管理端登录面板 props 装配 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`39e2a92`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceLoginPanelProps.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceLoginPanelProps.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里传给 `AdminLoginPanel` 的错误、提示、输入值与 loading props 已收口到共享 `adminWorkspaceLoginPanelProps` helper。
  - 新增 helper 单测，锁定登录面板 props 的装配契约。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认登录、退出与后台数据自举路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估登录面板事件或更完整的壳层 feature adapter，而不是重新把登录 props 组装写回组件层。

### 执行记录：FE-041（管理端壳层事件装配 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`77c4888`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceShellEvents.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceShellEvents.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里传给 `AdminShellFrame` 的 `logout / refresh / switch-view` 事件转发已收口到共享 `adminWorkspaceShellEvents` helper。
  - 新增 helper 单测，锁定壳层刷新、退出与视图切换事件的转发契约。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认后台导航、刷新、退出和模块渲染路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估登录面板 props 或更完整的壳层 feature adapter，而不是重新把壳层事件转发写回组件层。

### 执行记录：FE-041（管理端壳层 props 装配 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`ce4f832`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceShellProps.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceShellProps.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里传给 `AdminShellFrame` 的壳层 props 已收口到共享 `adminWorkspaceShellProps` helper。
  - 新增 helper 单测，锁定 active view / notice / loading / nav groups / profile 等壳层输入的装配契约。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认后台导航、刷新、退出和模块渲染路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估登录面板 props 或更完整的壳层 feature adapter，而不是重新把壳层 props 组装写回组件层。

### 执行记录：FE-041（管理端模块事件装配 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`74c06ed`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceModuleEvents.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceModuleEvents.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里 dashboard / moderation / governance 三组模块事件转发已收口到共享 `adminWorkspaceModuleEvents` helper。
  - 新增 helper 单测，锁定 dashboard 打开审核、moderation 动作/筛选、governance 动作/选中/筛选的转发契约。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认后台模块切换、筛选与治理动作路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估更完整的模块级 feature adapter，而不是重新把模块事件转发写回组件层。

### 执行记录：FE-041（管理端模块 props 装配 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`114dca6`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceModuleProps.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceModuleProps.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里 dashboard / moderation / governance 三组模块 props 装配已收口到共享 `adminWorkspaceModuleProps` helper。
  - 新增 helper 单测，锁定 dashboard 统计卡、moderation 列表 props 与 governance 动作/空态/筛选 props 的装配契约。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认后台模块切换、筛选与治理动作路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估模块级 feature adapter 或模块事件装配是否继续下沉，而不是重新把模块 props 组装写回组件层。

### 执行记录：FE-041（管理端确认弹层装配 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`e1ff2aa`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceConfirmDialogs.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceConfirmDialogs.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里 5 组确认弹层的 copy 解析与 dialog metadata 装配，已收口到共享 `adminWorkspaceConfirmDialogs` helper。
  - 新增 helper 单测，锁定 moderation / report / aiTask / template / user 五组 pending state 到 dialog stack 的装配契约。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认后台确认层的打开、取消与确认路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估模块级 feature adapter 或模块 props 装配是否继续下沉，而不是重新把确认弹层的 copy 组装写回组件层。

### 执行记录：FE-041（管理端待确认动作 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`2544e38`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspacePendingAction.ts`
  - 新增 `frontend-admin/src/views/adminWorkspacePendingAction.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里 `requestModerationAction(...)` 与 `requestGovernanceAction(...)` 的 pending action 打开逻辑已收口到共享 `adminWorkspacePendingAction` helper。
  - 新增 helper 单测，锁定 moderation 打开、治理动作分发到 report/material 路径，以及 invalid dispatch 的错误回写。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认治理动作的确认层打开路径未回退。
- 已执行验证：
...
- 后续影响：
  ...
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估 confirm dialog 的 copy/computed 组装和更完整的模块级 feature adapter 是否继续下沉，而不是重新把 pending action 打开逻辑写回组件层。

### 执行记录：FE-041（管理端确认状态 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`61f9677`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceConfirmState.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceConfirmState.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里确认弹层 reset/submit 映射表已收口到共享 `adminWorkspaceConfirmState` helper，页面层只保留 pending state、error state 和真实动作函数。
  - 新增 helper 单测，锁定全量 confirm reset、按 key 分发 submit，以及 pending 为空时的 noop 行为。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认治理动作确认层路径未回退。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从共享治理动作状态链推进到共享确认状态链，后台工作台壳层里残留的 confirm 编排进一步变薄。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估 pending action open state 与模块级 feature adapter 是否继续下沉，而不是重新把确认映射表写回组件层。

### 执行记录：FE-041（管理端治理动作状态 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`b7f8896`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceMutationState.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceMutationState.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里 moderation/governance 动作提交时残留的 loading、notice、confirm error、409 conflict 与 reload 编排已收口到共享 `adminWorkspaceMutationState` helper。
  - 新增 helper 单测，锁定资料治理成功回刷、409 conflict 标记、治理 invalid 分支、成功提示同步与治理冲突回写。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认治理动作确认层与状态路径未回退。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从共享数据加载链推进到共享治理动作状态链，后台工作台壳层里剩余的动作副作用也进一步变薄。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估确认弹层 open/reset state 与模块级 feature adapter 是否继续下沉，而不是重新把治理动作编排写回组件层。

### 执行记录：FE-041（管理端数据加载 helper 接线）

- 执行日期：2026-07-14
- 关联提交：`bd597a9`（RED）、待提交（GREEN）
- 实际变更：
  - 新增 `frontend-admin/src/views/adminWorkspaceDataLoad.ts`
  - 新增 `frontend-admin/src/views/adminWorkspaceDataLoad.test.ts`
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
- 完成证据：
  - `AdminWorkspaceView.vue` 里残留的 `refreshProfile()`、`loadOverview()`、`loadModeration()`、`loadGovernance()` 四条读取/加载链路已收口到共享 `adminWorkspaceDataLoad` helper，页面层只保留 state 绑定与 request adapter。
  - 新增 helper 单测，锁定 profile/overview 简单读取、moderation 的 403 清空与 governance 同 view 刷新失败时保留旧数据的行为。
  - 既有 `AdminWorkspaceView.test.ts` 页面级回归保持通过，确认登录、导航、刷新、403/409 状态与治理动作路径未回退。
- 已执行验证：
  ...
  - `git diff --check`
- 后续影响：
  - `FE-041` 现在继续从共享 lifecycle / popstate / refresh 执行链推进到共享数据加载链，后台工作台壳层里剩余的读取副作用进一步变薄。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估确认弹层状态与治理动作 state adapter 是否继续拆出页面壳层，而不是重新把数据加载分支写回组件层。

### 执行记录：FE-041（管理端 popstate 执行 helper 接线）
- 执行日期：2026-07-14
- 本轮完成：
  ...
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `handleAdminPopstate()` 改为消费共享 `runAdminWorkspacePopstate(...)` helper，而不是继续在组件里直接维护 reset / set view / load 的执行口。
  - 本轮不改 URL 解析协议、不改治理请求实现和会话协议，只把已稳定的 popstate 执行链继续从壳层组件下沉到共享 helper。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是 popstate 执行口本身，`AdminWorkspaceView.vue` 里仍保留 `loadActiveView(...)` 和更深层 `loadModeration / loadGovernance` 协调器。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合继续评估加载协调器与治理动作提交流程以外的剩余壳层绑定口，而不是重新把浏览器导航分支放回组件层。

### 执行记录：FE-041（管理端刷新执行 helper 接线）
- 执行日期：2026-07-14
- 本轮完成：
  - 更新 `frontend-admin/src/views/adminWorkspaceLifecycle.ts` 与 `adminWorkspaceLifecycle.test.ts`，补充共享 `buildAdminWorkspaceRefreshPlan(...)`，把当前 active view 刷新计划纳入统一 lifecycle 出口。
  - 新增 `frontend-admin/src/views/adminWorkspaceRefresh.ts` 与 `adminWorkspaceRefresh.test.ts`，收口刷新时是否触发当前视图重载的最小执行链。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `refreshActiveView()` 改为消费共享 refresh plan 与 refresh helper，而不是继续在组件里直接调用 `loadActiveView(activeView.value)`。
  - 本轮不改审核/治理加载实现、不改请求协议和会话协议，只把已稳定的刷新壳层入口继续从组件层下沉到共享 helper。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是 refresh 入口本身，`AdminWorkspaceView.vue` 里仍保留 `loadActiveView(...)` 本地协调器和更细的加载实现。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先评估 `loadActiveView(...)` 与更深层读写协调是否继续下沉，而不是在组件里重新扩张新的刷新分支。

### 执行记录：FE-041（管理端退出执行 helper 接线）
- 执行日期：2026-07-14
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceLogout.ts` 与 `adminWorkspaceLogout.test.ts`，收口退出时的 session/profile 清理、工作台 reset、默认 view 回退、无效化提示清空、持久化会话清除与退出提示同步顺序。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `logout()` 改为消费共享 `runAdminWorkspaceLogout(...)` helper，并补上登录态 `notice` 透传，确保退出后回到登录页时仍能看到“后台会话已清空。”提示。
  - 更新 `frontend-admin/src/components/admin/AdminLoginPanel.vue` 与 `AdminLoginPanel.test.ts`，为登录态视图补充非错误型 notice 展示出口。
  - 本轮不改 session store 协议、会话失效 prompt 语义或治理 API，只把已稳定的退出执行链继续从工作台组件下沉到共享 helper，并补齐退出提示在登录态页面上的可见性。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是 logout 最小执行顺序，`AdminWorkspaceView.vue` 里 `refreshActiveView()` 的共享刷新计划仍未进一步抽出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补 `refreshActiveView()` 的共享刷新计划，而不是把更多刷新分支继续留在组件里。

### 执行记录：FE-041（管理端视图切换执行 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceViewSwitch.ts` 与 `adminWorkspaceViewSwitch.test.ts`，收口导航切换时的工作台 reset、目标 view 应用、URL push/replace 与按需加载顺序。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `switchView(...)` 改为消费共享 `runAdminWorkspaceViewSwitch(...)` helper，而不是继续在组件里内联这段切换执行链。
  - 本轮不改路由解析规则、治理模块加载协议或刷新语义，只把已稳定的视图切换执行顺序继续从工作台组件下沉到共享 helper。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是视图切换最小执行顺序，`AdminWorkspaceView.vue` 里 `refreshActiveView()` 的共享刷新计划和 logout 分支的执行链仍未进一步抽出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补 `refreshActiveView()` 的共享刷新计划或把 logout 执行链继续收口，而不是把更多切换/刷新分支继续留在组件里。

### 执行记录：FE-041（管理端会话清空执行 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceSessionCleared.ts` 与 `adminWorkspaceSessionCleared.test.ts`，收口会话被清空后的工作台 reset、默认 view 回退、URL replace、错误清理与 notice 同步顺序。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `subscribeSession(...)` 里的 session-cleared 分支改为消费共享 `runAdminWorkspaceSessionCleared(...)` helper，而不是继续在组件里内联这段回退执行链。
  - 本轮不改 session store 协议、会话失效原因文案或治理 API，只把已稳定的会话清空执行语义继续从工作台组件下沉到共享 helper。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是 session-cleared 最小执行顺序，`AdminWorkspaceView.vue` 里 `refreshActiveView()` 的共享刷新计划和 session 变化下的其他协调仍未进一步抽出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补 `refreshActiveView()` 的共享刷新计划或把更多 session 变化协同继续收口，而不是把更多订阅分支继续留在组件里。

### 执行记录：FE-041（管理端挂载自举 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceMountBootstrap.ts` 与 `adminWorkspaceMountBootstrap.test.ts`，收口挂载时的目标 view 应用、profile 刷新与初始 view 加载顺序。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `onMounted()` 改为消费共享 `runAdminWorkspaceMountBootstrap(...)` helper，而不是继续在组件里内联挂载自举执行步骤。
  - 本轮不改挂载计划推导、URL 归一化或 session 生命周期协议，只把已稳定的挂载执行流程继续从工作台视图层下沉到共享 helper。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是挂载自举最小执行顺序，`AdminWorkspaceView.vue` 里 `refreshActiveView()` 的共享刷新计划和更完整的 session 变更协同仍未进一步抽出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补 active view 刷新计划 helper 或 session 变更协同 helper，而不是把更多刷新步骤继续留在组件里。

### 执行记录：FE-041（管理端治理动作提交流程 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminGovernanceMutationFlow.ts` 与 `adminGovernanceMutationFlow.test.ts`，收口四组治理动作共用的提交执行、dialog reset、reload view 与 409 conflict 信号。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `applyGovernanceRecordAction(...)` 改为消费共享 `runAdminGovernanceMutation(...)` helper，而不是继续在组件里直接串联 mutation meta、请求、对话框 reset 和治理视图回刷。
  - 本轮不改治理动作 API、确认弹层协议或工作台路由，只把已稳定的治理提交流程继续从工作台视图层下沉到共享 helper。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是治理动作提交最小编排，`AdminWorkspaceView.vue` 里更完整的 mount/read 协调和 active view 刷新计划仍未进一步抽出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补工作台级 mount/read 协调 helper 或 refreshActiveView 的共享刷新计划，而不是把更多视图级刷新分支继续留在组件里。

### 执行记录：FE-041（管理端审核动作元数据 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminModerationMutationMeta.ts` 与 `adminModerationMutationMeta.test.ts`，收口审核动作的 path、成功提示、资料治理回刷与 409 conflict 特例。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `applyModerationAction(...)` 改为消费共享 `resolveAdminModerationMutationMeta(...)` helper，而不是继续在组件里维护 path 拼接、notice 文案和资料治理特判。
  - 本轮不改审核动作 API、确认弹层协议或治理模块视图，只把已稳定的审核提交元数据继续从工作台视图层下沉到共享 helper。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是审核动作最小元数据，`AdminWorkspaceView.vue` 里治理动作提交后的刷新编排和更完整的 mount/read 协调仍未进一步抽出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补治理动作提交后的刷新编排 helper 或工作台级 mount/read 协调 helper，而不是把更多提交后分支继续留在组件里。

### 执行记录：FE-041（管理端工作台登录自举 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceBootstrap.ts` 与 `adminWorkspaceBootstrap.test.ts`，收口后台登录成功后的 session 持久化、profile 刷新与当前 view 加载顺序。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `login()` 改为消费共享 `runAdminWorkspaceLoginBootstrap(...)` helper，而不是继续在组件里串联登录后自举步骤。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.test.ts`，补一条登录成功后 dashboard 数据会完成自举加载的工作台回归。
- 已执行验证：
  ...
  - `git diff --check`
- 风险与后续：
  - 当前这轮先收口的是登录成功后的最小自举顺序，`AdminWorkspaceView.vue` 里与 mount/bootstrap 计划复用、治理提交后的刷新编排仍未进一步抽出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补工作台级 mount/read 协调 helper 或治理提交后的刷新编排 helper，而不是把更多登录后链路继续留在组件里。

### 执行记录：FE-041（管理端工作台加载编排 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceViewLoad.ts` 与 `adminWorkspaceViewLoad.test.ts`，收口 dashboard、moderation、governance 三类视图的 overview/moderation/governance 加载编排逻辑。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `loadActiveView(...)` 改为消费共享 `runAdminWorkspaceViewLoad(...)` helper，而不是继续在组件里维护这组三分支。
  - 本轮不改底层请求函数、治理加载协议或页面布局，只把已稳定的视图加载编排继续从工作台视图层下沉到共享 helper。
- 已执行验证：
  - `npm --workspace frontend-admin run test -- src/views/adminWorkspaceViewLoad.test.ts src/views/AdminWorkspaceView.test.ts`
  - `npm --workspace frontend-admin run typecheck`
- 风险与后续：
  - 当前这轮先收口的是视图级加载编排最小契约，`AdminWorkspaceView.vue` 里与登录提交、读取请求和治理提交相关的更细粒度协调逻辑仍未拆出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补工作台级 bootstrap/read 协调 helper，而不是把更多加载分支继续留在组件里。

### 执行记录：FE-041（管理端工作台生命周期 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceLifecycle.ts` 与 `adminWorkspaceLifecycle.test.ts`，收口工作台挂载自举、`popstate`、导航切换、会话失效回退与退出这几类生命周期计划。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让这些流程改为消费共享 lifecycle plan helper，而不是继续在工作台组件里内联 reset/sync/load 分支。
  - 本轮不改治理请求协议、模块加载计划或页面布局，只把已稳定的工作台生命周期协调逻辑继续从视图层下沉到共享 helper。
- 已执行验证：
  - `npm --workspace frontend-admin run test -- src/views/adminWorkspaceLifecycle.test.ts src/views/AdminWorkspaceView.test.ts`
  - `npm --workspace frontend-admin run typecheck`
- 风险与后续：
  - 当前这轮先收口的是生命周期计划最小契约，`AdminWorkspaceView.vue` 里与实际请求加载、登录提交和治理提交相关的协调逻辑仍未拆出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补工作台级 bootstrap/load 协调 helper，而不是把更多生命周期分支继续留在组件里。

### 执行记录：FE-041（管理端工作台路由定位 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceLocation.ts` 与 `adminWorkspaceLocation.test.ts`，收口工作台当前视图解析、非法后台路径归一化，以及 `push/replace` 两种历史同步逻辑。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让初始化视图、URL 归一化、`popstate` 回退、会话失效回退与手动切换导航都改为消费共享 `adminWorkspaceLocation` helper。
  - 本轮不改治理请求、模块加载计划或页面布局，只把已稳定的 URL/视图同步逻辑继续从工作台视图层下沉到共享 helper。
- 已执行验证：
  - `npm --workspace frontend-admin run test -- src/views/adminWorkspaceLocation.test.ts src/views/AdminWorkspaceView.test.ts`
  - `npm --workspace frontend-admin run typecheck`
- 风险与后续：
  - 当前这轮先收口的是工作台路由定位最小契约，`AdminWorkspaceView.vue` 里与 session 订阅、数据加载协调和治理提交相关的流程逻辑仍未拆出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补工作台级 session/bootstrap 协调 helper，而不是把更多 URL 分支继续留在工作台组件。

### 执行记录：FE-041（管理端工作台派生数据 helper 接线）
- 执行日期：2026-07-13
- 本轮完成：
  - 新增 `frontend-admin/src/views/adminWorkspaceDerivedData.ts` 与 `adminWorkspaceDerivedData.test.ts`，收口审核列表的帖子/资料拆分、本地筛选，以及治理列表的本地筛选和状态选项生成逻辑。
  - 更新 `frontend-admin/src/views/AdminWorkspaceView.vue`，让 `pendingPosts`、`pendingMaterials`、`visibleModerationItems`、`moderationStatusOptions`、`visibleGovernanceRows` 与 `governanceStatusOptions` 全部改为消费共享 `adminWorkspaceDerivedData` helper。
  - 本轮不改请求协议、治理动作、路由或页面布局，只把已稳定的派生数据计算继续从工作台视图层下沉到共享 helper。
- 已执行验证：
  - `npm --workspace frontend-admin run test -- src/views/adminWorkspaceDerivedData.test.ts src/views/AdminWorkspaceView.test.ts`
  - `npm --workspace frontend-admin run typecheck`
- 风险与后续：
  - 当前这轮先收口的是工作台派生数据最小契约，`AdminWorkspaceView.vue` 里与视图切换、URL 同步、请求协调和治理提交相关的流程逻辑仍未拆出。
  - 后续继续沿 `FE-041 / ADM-010` 推进时，更适合优先补工作台级视图状态/路由协调 helper，而不是把更多筛选与派生分支继续写回工作台组件。
...
- 本轮完成：
  ...
  - `docs/engineering/SEARCH_CONTRACT_AND_REGRESSION.md`、`docs/DEVELOPMENT.md`、`README.md` 与本执行记录已同步更新为新的分页边界说明。
- 已执行验证：
  - `npm --workspace frontend-user run test -- src/modules/search/SearchWorkspacePage.test.tsx`
  - `npm --workspace frontend-user run typecheck`
  - `npm run build:user`
- 风险与后续：
  - 本轮仍然没有把“全部类型”视图升级成真正的服务端统一分页，只是把已有 `nextOffset` 能力放开到按分组续取。
  - 后续若要继续推进 `SE-020`，更适合补跨批次排序语义、空结果建议和更完整的 grouped cursor，而不是重新退回仅单一类型可续取的旧交互。
...
- 本轮完成：
  ...
  - `frontend-user/src/api/search.ts`、`src/api/types.ts`、`src/modules/search/SearchWorkspacePage.tsx` 与页面/API 测试已接上新的 `offset/nextOffset` 契约；当某个分组仍有后续结果时，页面可以继续显示“继续加载更多...”并把下一批结果追加到当前组中。
  - `docs/engineering/SEARCH_CONTRACT_AND_REGRESSION.md`、`README.md` 与 `PROJECT_LOG.md` 已同步记录当前分页边界：分组可继续请求下一批，但“全部类型”视图仍不是完整的后端真分页。
- 已执行验证：
  - `cd backend && go test ./internal/modules/search/service ./internal/modules/search/handler`
  - `npm --workspace frontend-user run test -- src/api/searchShare.test.ts src/modules/search/SearchWorkspacePage.test.tsx`
- 风险与后续：
  - 当前 `offset/nextOffset` 已收口到 grouped payload 与按分组独立续取路径；但“全部类型”视图仍以首批聚合结果为主，没有补完整的后端统一分页编排。
...
### 执行记录：ANKI-070（复习工作区卡组导入导出起步）

- 执行日期：2026-07-15
- 执行分支/提交：`master` / 未提交
- 实际变更：
  - 新增 `frontend-user/src/modules/review/deckImportExport.ts`
  - 新增 `frontend-user/src/modules/review/deckImportExport.test.ts`
  - 更新 `frontend-user/src/modules/review/ReviewWorkspacePage.tsx`
  - 更新 `frontend-user/src/modules/review/ReviewWorkspacePage.test.tsx`
  - 更新 `frontend-user/src/styles/studio-workspaces.css`
  - 更新 `docs/engineering/CODEX_BACKLOG.md`
  - 更新 `PROJECT_LOG.md`
- 完成证据：
  - 复习管理面板里的“卡片”页签已支持将当前卡组导出为本地 JSON / CSV 文件，不再只能依赖页面内存态浏览卡片内容。
  - 当前卡组已支持从本地 JSON / CSV 文件批量导入卡片，导入逻辑会复用既有 `bulkCreateDeckCards(...)` 接口并自动按 20 条分片提交。
  - 导入导出解析逻辑已从页面层抽到独立 `deckImportExport` helper，覆盖标签、基础来源字段与 CSV quoted field 解析，不把格式细节继续堆在 `ReviewWorkspacePage.tsx` 里。
- 已执行验证：
  ...
  - `npm run build:user`
- 未执行验证及原因：
  - 未运行后端测试：本工作包完全复用既有 `listDeckCards / bulkCreateDeckCards` 接口，没有修改 Go 侧 card 模块实现。
- 兼容性/迁移说明：
  - 本轮导入导出先落在用户端复习工作区，本地文件由浏览器发起下载与读取；没有新增后端导入导出路由，也没有数据库迁移。
  - CSV 导入导出当前只覆盖 `front / back / cardType / tags / sourceType / sourceId` 六个基础字段；更完整的 `sourceMetadata` 与失败报告仍建议优先使用 JSON。
- 已知风险：
  - 浏览器下载在 Vitest/JSDOM 中会出现 “Not implemented: navigation to another Document” 的环境提示，但不影响实际断言与构建结果。
  - 当前导入前还没有独立预检面板、重复卡片检测或逐条错误报告；大批量失败时仍主要依赖现有接口返回错误。
- 下一建议任务：
  - `ANKI-070` 继续补导入预检、失败报告与后端导入导出接口
### 执行记录：ANKI-070（后端导入导出接口起步）
...
- 实际变更：
  ...
  - 更新 `backend/internal/modules/card/router/router.go`
  - 更新 `backend/internal/modules/card/service/service.go`
  - 新增 `backend/internal/modules/card/service/import_export.go`
  - 更新 `backend/internal/modules/card/handler/handler_test.go`
  - 更新 `backend/internal/modules/card/service/list_cards_filters_test.go`
  - 更新 `frontend-user/src/api/review.ts`
  - 更新 `frontend-user/src/api/types.ts`
  ...
  - 更新 `PROJECT_LOG.md`
- 完成证据：
  - 后端新增 `GET /decks/:id/export?format=json|csv`，会统一返回文件名、MIME、内容、卡片数与导出时间，避免前端再自己拼 portable artifact。
  - 后端新增 `POST /decks/:id/import`，可直接接收 `filename + content` 的 JSON/CSV 卡片文件内容并写入当前卡组。
  - 复习工作区的“导出 JSON / 导出 CSV / 导入卡片文件”入口现已改为走后端接口，而不是只依赖前端本地拼装或本地解析后再分批直写。
- 已执行验证：
  - `go test ./internal/modules/card/handler ./internal/modules/card/service`
  - `go test ./internal/modules/card/...`
  ...
  - `npm run verify:docs`
- 未执行验证及原因：
  - 未运行更大范围的 review e2e：本轮只补导入导出接口与页面接线，没有改复习评分/调度主路径。
- 兼容性/迁移说明：
  - 本工作包新增的是轻量导入导出接口，不改变既有 `bulkCreateDeckCards(...)`、复习评分接口和卡片列表过滤契约。
  - 当前导入仍然通过 `filename + content` 的 JSON 请求体提交，不含 multipart 上传、预检草稿或失败明细中心；这是原型阶段的最小服务端承接边界。
- 已知风险：
  - 服务端导入当前只做最小解析与 200 张上限控制，还没有重复卡片检测、逐条失败报告和导入预览。
...
- 实际变更：
  ...
  - 更新 `PROJECT_LOG.md`
- 完成证据：
  - 共享 `ConfirmDialog` 现已支持结构化 `description` 内容，确认弹层不再被限制为单行纯文本说明。
  - 复习工作区的导入预检弹层现在会直接展示摘要、`重复卡片 N 条` 与 `失败记录 N 条` 两组明细，并保留每条记录的行号、卡片正面与错误原因。
  - 预检成功且仍有可导入卡片时，页内消息不再重复回显同一条摘要，避免确认弹层和页面提示同时出现相同文案。
- 已执行验证：
  ...
  - `git diff --check`
- 未执行验证及原因：
  - 未运行 review e2e：本轮只补前端预检结果的展示层与共享确认框契约，没有改动评分、调度或后端导入逻辑。
- 兼容性/迁移说明：
  - 本轮没有新增数据库迁移，也没有改变导入导出接口、请求体字段或服务端返回结构；调整仅发生在共享确认框可接收 `ReactNode` 描述内容，以及复习工作区如何组织已有预检数据的展示。
- 已知风险：
  - 当前预检明细仍依赖服务端返回的轻量样本列表，尚未提供更完整的逐条修复建议、批量忽略策略或独立导入结果中心。
  - 共享确认框虽然已经支持结构化内容，但其他调用点目前仍主要使用纯文本说明，后续如继续扩展富内容确认流，需要留意弹层宽度和移动端排版。
- 下一建议任务：
  - `ANKI-070` 继续补独立导入结果面板与更细粒度的修复建议
### 执行记录：ANKI-070（最近一次导入结果面板）
...
- 实际变更：
  ...
  - 更新 `PROJECT_LOG.md`
- 完成证据：
  - 复习工作区现在会在卡组导入区持久展示“最近一次导入结果”，覆盖文件名、预检/导入阶段和结构化摘要。
  - 预检弹层打开时，持久面板只保留摘要，避免与确认弹层里的重复/失败明细重复渲染；确认导入完成后，面板继续保留最终导入结果与明细。
  - 页面级回归已锁定 `cards.json · 预检结果` 与 `cards.json · 导入结果` 两种状态，避免导入结果只能靠瞬时消息查看。
- 已执行验证：
  - `npm --workspace frontend-user run test -- src/modules/review/ReviewWorkspacePage.test.tsx`
  - `npm --workspace frontend-user run test -- src/design-system/primitives/ConfirmDialog.test.tsx src/modules/review/ReviewWorkspacePage.test.tsx`
  - `npm --workspace frontend-user run typecheck`
  ...
  - `git diff --check`
- 未执行验证及原因：
  - 未运行 review e2e：本轮只补导入结果持久展示，不改评分或调度主路径。
- 兼容性 / 迁移说明：
  - 无接口、请求体或数据结构变更；调整仅发生在复习工作区如何组织既有导入预检/导入结果。
- 已知风险：
  - 当前结果面板仍只保留最近一次导入结果，没有历史记录、批量修复建议或独立结果中心。
- 后续影响：
  - `ANKI-070` 现在从“确认前可看明细”进一步推进到“确认后也能回看结果”，导入链路不再只依赖瞬时消息和弹层。
  - 后续更适合继续补逐条修复建议和结果历史，而不是立刻扩到更重的导入协议。
