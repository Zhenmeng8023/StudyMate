...
| ANKI-050 | IN_PROGRESS | 来源驱动制卡闭环 | ANKI-010, WB-030 | reader/note/graph/ai/card | 批注来源卡片现已补齐 `sourceMetadata` 通路，能在草稿确认、卡片创建与复习队列中保留 reader 上下文并回跳到原批注；后续继续补 PDF 锚点、图谱节点与更通用的 SourceLink 抽象。 |
...
| LC-010 | IN_PROGRESS | 主学习闭环演示路径收口 | WB-033, ANKI-030, FE-030 | material/reader/note/graph/card/review/AI | “资料上传 -> PDF 阅读 -> 高亮批注 -> 摘录池 -> 笔记块 -> CardNote -> 模板生成闪卡 -> Anki 式复习 -> 图谱熟练度回写”已开始按真实路径收口；当前已补上图谱来源卡片 `/review?card=` 深链定位、复习页内对可直达来源工作台的回看入口，以及批注来源卡片到 reader 批注位置的真实回跳，后续继续补 PDF 锚点、失败状态与复习反馈清晰。 |
