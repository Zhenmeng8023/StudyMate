...
## 2026-07-15 设计说明书复核后的快速原型校准

### 核验结论

对照根目录《学伴项目-设计说明书.md》，当前仓库已经跨过 Phase 1-4，并且 Phase 5 图谱产品化、Phase 6 Anki/AI 融合都已出现可运行切片。说明书里“每个模块达到对应赛道可用产品深度”的目标仍成立，但当前执行计划需要从“平均补专业能力”校准为“先把主演示学习闭环跑通，再逐段加深”。

当前真实进展可以这样判断：

- 平台基础、社区/资料、阅读/笔记、图谱 MVP 已可作为原型底座。
- 图谱已具备 graph-core 契约、导入导出、验证、快照、冲突处理和布局预览能力，下一步不应继续只补细节按钮，而应把图谱作为学习闭环里的可追溯来源与反馈承载面。
- 复习模块已不再只是 Deck/Card CRUD：今日队列、SM-2、跳过、暂停、埋藏、撤销评分、来源回跳、卡片浏览器、标签批量管理、JSON/CSV 导入导出和导入预检都已起步；但正式的 CardNote / NoteType / Template / GeneratedCard 模型仍未落地。
- AI 已有草稿、任务、用量和来源回跳切片，适合先服务“解释资料 -> 生成草稿 -> 确认进卡片/图谱”的主演示路径，而不是先扩成完整 AI 产品中心。
- 搜索已经有 MySQL fallback、分组结果、真实计数和分组续取；Meilisearch 仍应作为可替换索引的下一阶段，而不是打断当前闭环原型。
- 后台治理已从占位进入可操作状态，下一步应优先补支撑演示和安全边界的治理动作、审计和失败提示，而不是扩张课程/LMS。
...
1. **LC-010 主学习闭环优先**
   先交付一条可演示路径：资料上传/选择 -> PDF 阅读与批注 -> 笔记摘录 -> 图谱节点 -> 生成卡片 -> 复习评分 -> Dashboard/图谱看到薄弱反馈。允许先复用现有 `Card(front/back)` 和 `sourceMetadata`，不要等待完整 Anki 模型。
2. **ANKI-000/010/020 做“兼容层设计 + 最小落地”**
   不是推翻当前 Card 模块，而是在现有卡片上补 CardNote、NoteType、Template 和队列状态的兼容设计。第一阶段只需要 Basic、Reverse、Cloze 的最小模板和旧卡片兼容读取。
3. **WB-033 / ANKI-060 补最小反馈回写**
   先把复习结果回写为图谱节点/来源级 `mastery` 或等价摘要，支持 Dashboard 和图谱 Inspector 展示薄弱知识点。复杂学习画像后置。
4. **SE-020 / ADM-011 服务主路径**
   搜索先补空结果建议、来源跳转和稳定分页体验；后台先补与演示、安全直接相关的治理备注、审计可读性和失败重试。
5. **GPH-040 只做阻止膨胀的拆分**
   图谱控制器拆分继续做，但目标是释放下一步闭环功能的实现速度；不为了纯架构完美而长时间停止产品切片。

### 明确后置

以下仍保留在设计说明书愿景中，但不进入当前快速原型主线：

- 课程/LMS、班级、小组协作、作业评测。
- Tauri 桌面端、多人实时协作、PWA 离线。
- 完整工程图谱套件、代码分析图、PlantUML/OpenAPI/SQL 全量双向转换。
- 向量数据库、完整 RAG、Elasticsearch 分析系统。
- 完整 `.apkg` 兼容、媒体同步和插件级 Anki 生态。
...
### 2026-07-09 FE-041 共享 Select 接入笔记与阅读表单

- `@studymate/ui` 已新增共享 `Select`，先统一 `ds-select` class 与最小 `invalid` 错误态语义，让共享层开始承接最常见的下拉选择契约。
...
### 2026-07-09 FE-041 共享 ConfirmDialog 接入笔记删除确认

- `@studymate/ui` 已新增共享 `ConfirmDialog`，先统一确认标题、说明文案、取消/确认动作、`danger` 变体、确认中禁用与错误提示这组最小确认交互契约。
- 用户端 `NotesPage` 已把删除笔记从原生 `window.confirm(...)` 改为共享 `ConfirmDialog`，说明 `FE-041` 已开始从页面骨架进一步收口到真实 destructive action 的确认流程。
- 下一步更值得继续推进图谱工作区里的“重载最新图谱 / 删除图谱”确认路径，或者沿后台治理动作把筛选条和确认框一起纳入共享状态语义。

### 2026-07-09 FE-041 共享 ConfirmDialog 扩展到图谱工作区主路径

- 图谱工作区里的“放弃本地并重载最新图谱”与“删除当前图谱”现在都已改为先弹共享 `ConfirmDialog`，不再直接调用原生 `window.confirm(...)`。
...
### 2026-07-09 ADM-011 AI 任务治理动作起步

- 后端已补齐 `/api/v1/admin/ai/tasks/:id/retry` 与 `/api/v1/admin/ai/tasks/:id/cancel`，服务层会受控处理 `failed -> pending` 与 `pending -> cancelled` 两条状态迁移，并写入 `admin.handle.ai_task` 审计事件。
