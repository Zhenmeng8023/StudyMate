...
| E2E smoke | 图谱工作区真实预览环境下的桌面与窄屏加载、保存、导入、历史、布局预览、导出状态、权限路径、失败态、版本冲突处理，以及“应用已标记取舍 -> 生成合并草稿 -> 以 rebased document 再次保存”的主路径 | `e2e/v1-graph-workspace.spec.ts` | `npm run test:graph:conflicts:e2e` |
...
- 当前固定入口已经覆盖对象级取舍应用后的真实保存路径，并会断言第二次 `batch-save` 使用 rebased `document.version`；后续若再扩取舍能力，应继续把这类真实保存链路接进同一入口。
- 它还没有覆盖：
  - 更完整的桌面 / 窄屏冲突组合矩阵
